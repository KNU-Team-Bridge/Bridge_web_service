...
function MeetingPage() {
  ...
  const [isMicRec, setMicRec] = useState(false);
  const [isVideoRec, setVideoRec] = useState(false);
  ...
  const toggleVideoRec = () => {
    setVideoRec(!isVideoRec);
  };
  ...
  //let captureAndSendImage = useRef(null);
  ...
  useEffect(() => {
    ...
    newSocket.on('imageProcessed', (data) => {
    ...
    function captureAndSendImage(videoElement) {
      ...
      // 이미지 데이터를 서버로 전송
      socket.emit('imageCaptured', dataURL);
    }

    intervalId.current = setInterval(() => {
      console.log('hello');
      if (localVideoRef.current && isCameraOpen) {
        captureAndSendImage(localVideoRef.current);
      }
    }, 1000);
  ...
  }, [isCameraOpen, isMicOpen, intervalId, socket]);
    ...
    <div className='meetpageBackground'>
      <div className='head-body-functions'>
        ...
        <div className='functions'>
          ...
          <div className='micRecDiv'>
            {isMicRec ? (
              <button className='micRecBtn' onClick={toggleMicRec}>
                <PiEar className='earIcon' />
              </button>
            ) : (
              <button className='micRecBtnOff' onClick={toggleMicRec}>
                <PiEar className='earIcon' />
              </button>
            )}
          </div>
          <div className='videoRecDiv'>
            {isVideoRec ? (
              <button className='videoRecBtn' onClick={toggleVideoRec}>
                <FaHandPaper className='handIcon' />
              </button>
            ) : (
              <button className='videoRecBtnOff' onClick={toggleVideoRec}>
                <FaHandPaper className='handIcon' />
              </button>
            )}
          </div>
